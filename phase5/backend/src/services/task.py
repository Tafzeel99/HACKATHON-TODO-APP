...
class TaskService:
    ...
    def __init__(self, session: AsyncSession):
        ...
        self._scheduler = get_job_scheduler()
    ...
    async def create(self, user_id: uuid.UUID, data: TaskCreate) -> Task:
        ...
        # Schedule reminder if reminder_at is set
        if task.reminder_at:
    ...
    async def update(self, task: Task, data: TaskUpdate) -> Task:
        ...
        await self._publish_task_event("updated", task)

        # Reschedule reminder if reminder_at changed
        if data.reminder_at is not None:
            await self._scheduler.cancel_reminder(str(task.id))
            if task.reminder_at:
                await self._schedule_reminder(task)
    ...
    async def delete(self, task: Task) -> None:
        ...
        await self._publisher.publish_task_event("deleted", task_id, task_data, user_id)

        # Cancel any scheduled reminder
        await self._scheduler.cancel_reminder(task_id)
    ...
    async def toggle_complete(self, task: Task) -> tuple[Task, Task | None]:
        ...
        await self._publish_task_event("completed", task)
    ...
    # --- Event helpers (Phase 5) ---

    def _task_to_dict(self, task: Task) -> dict:
        """Convert a Task model to a serializable dict for events."""
        return {
            "id": str(task.id),
            "title": task.title,
            "description": task.description,
    ...
    async def _publish_task_event(self, event_type: str, task: Task) -> None:
        """Publish a task event (fire-and-forget)."""
        try:
            ...
            logger.error("Failed to publish %s event for task %s: %s", event_type, task.id, e)

    async def _schedule_reminder(self, task: Task) -> None:
        """Schedule a Dapr Job for the task's reminder_at time."""
        if not task.reminder_at:
            return
        try:
            await self._scheduler.schedule_reminder(
                task_id=str(task.id),
                remind_at=task.reminder_at.isoformat() + "Z",
                user_id=str(task.user_id),
                title=task.title,
            )
        except Exception as e:
            logger.error("Failed to schedule reminder for task %s: %s", task.id, e)
